#!/bin/bash
# run_test_cli.sh - Script to run the CLI tool in the tests directory

# Set script directory and app path
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_DIR="/Users/Roshan.Abady/Dev/ausbiz"
CLI_APP="$PROJECT_DIR/tests/cli.py"
...
echo -e "${BLUE}🧪 Australian Business Names Test CLI Launcher${NC}"
echo "=================================================="

# Check if the CLI app file exists
if [ ! -f "$CLI_APP" ]; then
    echo -e "${RED}❌ Error: Test CLI app not found at $CLI_APP${NC}"
    exit 1
fi
...
echo -e "${GREEN}🚀 Running test CLI app...${NC}"
python "$CLI_APP" "$@"

# Show command help if no arguments provided
if [ $# -eq 0 ]; then
    echo -e "${YELLOW}💡 Tip: You can specify commands, e.g.:${NC}"
    echo -e "   ./run_test_cli.sh search \"ACME\" --display barchart"
    echo -e "   ./run_test_cli.sh stats"
    echo -e "   ./run_test_cli.sh --help"
    echo ""
    echo -e "${BLUE}📘 This is the test version of the CLI in the tests directory${NC}"
fi
